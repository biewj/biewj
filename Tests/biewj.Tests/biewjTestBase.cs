...
namespace biewj.Tests
{
    public abstract class biewjTestBase : AbpIntegratedTestBase<biewjTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected biewjTestBase()
        {
        ...
        protected void UsingDbContext(Action<biewjDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<biewjDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<biewjDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<biewjDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<biewjDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<biewjDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<biewjDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<biewjDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<biewjDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<biewjDbContext, Task<T>> func)
        {
            T result;
